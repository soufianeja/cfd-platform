...
export default function CfdProjectEdit() {
  ...
  const { data: project, isLoading } = useQuery({
    queryKey: ['cfd-project-edit', slug],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (project) {
      setForm({
        ...
        mesh_cells: project.mesh_cells ?? '',
        reynolds_number: project.reynolds_number ?? '',
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-8 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Select label="Software" field="software" options={SOFTWARE_OPTIONS} value={form.software} onChange={set('software')} error={errors.software} />
            <Select label="Simulation Type" field="simulation_type" options={SIMULATION_TYPES} value={form.simulation_type} onChange={set('simulation_type')} error={errors.simulation_type} />
