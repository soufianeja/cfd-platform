    ...
    <nav className="bg-white border-b shadow-sm sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          <Link to="/predict" className="text-sm font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1">
          ...
          {token ? (
            <>
              ...
              {user?.role === 'admin' && (
                <Link to="/admin" className="text-sm font-semibold text-purple-600 hover:text-purple-700">
                  Admin
                </Link>
              )}
              {user?.role === 'reviewer' && (
                <Link to="/reviewer" className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
